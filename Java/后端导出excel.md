```java
@GetMapping("/doExportMaintainancePlan")
@ResponseBody
public void doExportMaintainancePlan( HttpServletResponse response, String dcName, Integer category, String planId) {

    List<MaintainancePlanComment> comments = new ArrayList<>();
    List<MaintainancePlanDetail> planDetails = new ArrayList<>();

    HSSFWorkbook workbook = new HSSFWorkbook();
    HSSFSheet sheet = PlanExportUtils.initFileFrame(workbook);
    try {
        MaintainancePlan plan = this.getService().getPlanByDc(dcName, category, planId, 0);
        if (plan != null) {
            comments = plan.getComments();
            planDetails = plan.getDetailList();
        }
        List<Map<String,Object>> serviceMapList = PlanExportUtils.assemblePlanValues(comments,planDetails);

        sheet = PlanExportUtils.setValueIntoCells(serviceMapList,sheet,workbook);
        int sheetLastRowNum = sheet.getLastRowNum();
        for(int x=0;x<sheetLastRowNum;x++){
            HSSFRow row =  sheet.getRow(x);
            if(row!=null){
                calcAndSetRowHeigt(sheet.getRow(x));
            }
        ...
        log.error("caught DataAccessException during get data process", e);
    } catch (IOException e) {
        log.error("caught IOException during workbook write outputStream", e);
    } finally {
        try {
            ...
            log.error("doExportMaintainancePlan() caught IOException", e);
        }
    }
}
```
        ...
        for (int cellIndex = sourceRow.getFirstCellNum();
            ...
            if (contentList.size() > 1) {
                ...
                for (String content : contentList) {
                    ...
                    if (num > 1.0) {
                        tempCountWidth =( (int)num +3) * cellWidth;
                    }else {
